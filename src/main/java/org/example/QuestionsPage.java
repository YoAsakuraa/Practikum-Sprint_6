...
public class QuestionsPage  extends BasePage {
    ...
    //Не стал удалять эти локаторы , что бы показать какой был ход мысли  ,  потом пришел к мысли о том что лучше реализовать универсальные что бы была параметаризация
